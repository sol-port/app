...
export function PortfolioConfirmation({ result, onConfirm, onRetry, walletAddress }: PortfolioConfirmationProps) {
  ...
  const improvements = [
    {
      ...
      title: t("portfolio.increase").replace("{amount}", String(modelInput.periodic_contributions_amount || 150)),
      description: `${t("portfolio.fundedRatio")}: ${
        assetAnalysis?.total_profit ? Math.round(assetAnalysis.total_profit + 8) : 108
      }% / ${t("portfolio.successProbability")}: ${
        modelOutput.success_probability ? Math.round(modelOutput.success_probability * 100) : 92
      }%`,
    },
    {
      id: "extend_period",
      ...
      description: `${t("portfolio.fundedRatio")}: ${
        assetAnalysis?.total_profit ? Math.round(assetAnalysis.total_profit + 12) : 112
      }% / ${t("portfolio.successProbability")}: ${
        modelOutput.success_probability ? Math.round((modelOutput.success_probability + 0.03) * 100) : 95
      }%`,
    },
  ]
    ...
    <div className="bg-[#161a2c] rounded-lg p-6 mb-6">
      ...
      {isMockPortfolio && (
        <div className="bg-blue-900/30 border border-blue-500 text-blue-100 px-4 py-2 rounded-md mb-4 flex items-center">
          ...
          <span className="text-sm">
            {t("portfolio.mockNotice") || "This is a sample portfolio created for demonstration purposes."}
          </span>
        </div>
      )}
      ...
      <p className="text-solport-textSecondary mb-6">
        {modelOutput.portfolio_name ? (
          <span className="font-medium text-lg">{modelOutput.portfolio_name}</span>
        ) : (
          t("portfolio.description").replace("{simulations}", String(modelInput.initial_investment || 10000))
        )}
      </p>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="bg-[#1a1e30] rounded-lg p-4">
          <h3 className="text-lg font-medium mb-4">{t("portfolio.composition")}</h3>
          <div className="h-64">
        ...
        <div className="bg-[#1a1e30] rounded-lg p-4">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-solport-textSecondary">{t("portfolio.expectedReturn")}</span>
            ...
            <div className="flex justify-between items-center">
              <span className="text-solport-textSecondary">{t("portfolio.expectedVolatility")}</span>
              <span className="text-lg font-medium">{(modelOutput.expected_volatility * 100).toFixed(1)}%</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-solport-textSecondary">{t("portfolio.fundedRatio")}</span>
              <span className="text-lg font-medium text-red-500">
                {assetAnalysis?.total_profit || (modelOutput.success_probability * 100).toFixed(0)}%
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-solport-textSecondary">{t("portfolio.successProbability")}</span>
            ...
            {modelInput.goal_date && (
              <div className="flex justify-between items-center">
                <span className="text-solport-textSecondary">{t("portfolio.targetDate") || "Target Date"}</span>
                <span className="text-lg font-medium">{formatDate(modelInput.goal_date)}</span>
              </div>
            )}
            {modelInput.investment_purpose && (
              <div className="flex justify-between items-center">
                <span className="text-solport-textSecondary">{t("portfolio.purpose") || "Investment Purpose"}</span>
          ...
          <div className="mt-6 p-4 bg-[#242b42] rounded-lg">
            <p className="text-sm">
              {t("portfolio.analysis")
                .replace("{percent}", (modelOutput.success_probability * 100).toFixed(0))
                .replace("{probability}", (modelOutput.success_probability * 100).toFixed(0))}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#1a1e30] rounded-lg p-4 mb-6">
        <h3 className="text-lg font-medium mb-4">{t("portfolio.improvements")}</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {improvements.map((improvement) => (
            ...
            >
              ...
              <p className="text-sm text-solport-textSecondary">{improvement.description}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="text-sm text-solport-textSecondary">
          {modelOutput.portfolio_id && modelOutput.portfolio_id.startsWith("MOCK")
            ? `Portfolio #${modelOutput.portfolio_id.substring(4)}`
            : `Portfolio #${modelOutput.portfolio_id || "42E...8d9B"}`}
        </div>

        {modelOutput.creation_date && (
          <div className="text-sm text-solport-textSecondary">Created: {formatDate(modelOutput.creation_date)}</div>
        )}
      </div>
      ...
      <div className="flex justify-end space-x-4">
        <Button
          variant="outline"
          className="border-solport-accent text-solport-accent hover:bg-solport-accent hover:text-white"
          onClick={onRetry}
        >
          <RefreshCw className="mr-2 h-4 w-4" />
          {t("portfolio.restart")}
        </Button>
        <Button className="bg-solport-accent hover:bg-solport-accent2" onClick={onConfirm}>
